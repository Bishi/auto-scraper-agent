...
The build pipeline does not inject any environment variables or secrets into the binary. You can verify this in `.github/workflows/release.yml` — Windows signing secrets (`WINDOWS_CERTIFICATE`, `WINDOWS_CERTIFICATE_PASSWORD`) are for Authenticode code signing and exist only for the duration of the CI job. macOS CI currently uses ad-hoc signing for internal-test DMGs, so no Apple Developer ID or notarization secrets are required yet.
...
| Binary (`.exe` / `.app` / `.dmg`) | - | Nothing sensitive |
...
The `agent.json` file is created by the sidecar under `~/.auto-scraper/agent.json` and is intended to be readable only by the current user. On Windows shared-machine deployments, consider restricting permissions further (`icacls "%USERPROFILE%\.auto-scraper\agent.json" /inheritance:r /grant:r "%USERNAME%":F`).
...
6. Builds the Tauri installer:
   ...
   - macOS: DMG with ad-hoc signing for internal testing.
...
7. Uploads the installer assets to a versioned GitHub Release.
