...
If the server revokes or abandons the saved device credentials, it may close the
WebSocket with code `4003` or return `401` with `terminal: true` from agent API
calls. The agent treats either signal as terminal: it stops heartbeat and
WebSocket reconnect loops, clears saved device credentials, and waits for the
user to pair again.
