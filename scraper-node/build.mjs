...
const tauriDest = join(binariesDir, tauriName);
copyFileSync(outExe, tauriDest);
if (!isWin) {
  chmodSync(tauriDest, 0o755);
}
...
console.log(`  Then copy headless shell → src-tauri/binaries/chromium-headless-shell-${triple}${isWin ? ".exe" : ""}`);
