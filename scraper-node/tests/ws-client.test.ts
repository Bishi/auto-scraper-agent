...
describe("AgentWebSocketClient", () => {
  ...
  it("mints a token, connects, logs, and heartbeats on connect", async () => {
    ...
    expect(loggerMock.info).toHaveBeenCalledWith(
      "[ws] Connected to agent WebSocket",
    );
    expect(loggerMock.info).toHaveBeenCalledWith(
      "[ws] Server accepted connection",
    );
  ...
  it("stops reconnecting when token minting reports terminal credentials", async () => {
    vi.useFakeTimers();
    vi.stubGlobal("WebSocket", FakeWebSocket);
    ...
    expect(client.getWsToken).toHaveBeenCalledTimes(1);
    expect(FakeWebSocket.instances).toHaveLength(0);
  });

  it("stops reconnecting after a terminal server close", async () => {
    vi.useFakeTimers();
    vi.stubGlobal("WebSocket", FakeWebSocket);
    const onTerminalCredentials = vi.fn();
    const client = {
    ...
    FakeWebSocket.instances[0]!.close(4003, "forgotten");
    await vi.advanceTimersByTimeAsync(60_000);

    expect(onTerminalCredentials).toHaveBeenCalledWith("forgotten");
  ...
  it("keeps successful planned token refresh silent", async () => {
    ...
    expect(loggerMock.info).not.toHaveBeenCalledWith(
      "[ws] Refreshing WebSocket token",
    );
    expect(loggerMock.info).not.toHaveBeenCalledWith(
      "[ws] Closed code=4001 reason=token refresh",
    );
    expect(loggerMock.info).not.toHaveBeenCalledWith(
      "[ws] Connected to agent WebSocket",
    );
  ...
  it("warns when token refresh reconnect fails and logs recovery", async () => {
    ...
    expect(loggerMock.info).toHaveBeenCalledWith(
      "[ws] Reconnected after token refresh failure",
    );
    wsClient.stop();
  });
