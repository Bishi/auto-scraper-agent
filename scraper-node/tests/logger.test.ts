...
import { AGENT_LOG_BUFFER, agentLogger, pushScraperLogs, sanitizeScraperLogEntry, SCRAPER_LOG_BUFFER } from "../src/logger.js";
...
describe("central agent log redaction and spool", () => {
  ...
  it("classifies central logs into structured components", async () => {
    const pushLogs = vi.fn().mockResolvedValue({ ok: true, accepted: 3, duplicates: 0, invalid: 0 });
    configureCentralLogUpload({ pushLogs } as unknown as Parameters<typeof configureCentralLogUpload>[0]);
    enqueueCentralAgentLog({
      level: 30,
      time: Date.UTC(2026, 4, 1, 9, 3, 52),
      msg: "[ws] Connected to agent WebSocket",
    });
    enqueueCentralAgentLog({
      level: 30,
      time: Date.UTC(2026, 4, 1, 9, 3, 53),
      msg: "[heartbeat] Heartbeat recovered",
    });
    enqueueCentralAgentLog({
      level: 30,
      time: Date.UTC(2026, 4, 1, 9, 3, 54),
      msg: "[command] Logs refreshed on request",
      component: "command",
      event: "logs_refresh_requested",
    });

    await flushCentralLogs();

    expect(pushLogs).toHaveBeenCalledWith([
      expect.objectContaining({ component: "ws" }),
      expect.objectContaining({ component: "heartbeat" }),
      expect.objectContaining({ component: "command", event: "logs_refresh_requested" }),
    ]);
  });

  it("keeps central:false agent logs local only", async () => {
    agentLogger.info({ central: false }, "[agent] Local-only protocol detail");
    await vi.waitFor(() => expect(AGENT_LOG_BUFFER.at(-1)?.msg).toBe("[agent] Local-only protocol detail"));
