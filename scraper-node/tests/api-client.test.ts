...
describe("describeAgentApiError", () => {
  ...
  it("passes through API errors unchanged", () => {
    expect(
      describeAgentApiError(
        new Error("API POST /api/agent/results -> 500: boom"),
      ),
    ).toBe("Error: API POST /api/agent/results -> 500: boom");
  });
  ...
  it("classifies 503 API responses as transient without exposing the body", () => {
    ...
    expect(describeAgentApiError(err)).toBe(
      "server temporarily unavailable; will retry",
    );
  });
...
describe("AgentApiClient", () => {
  ...
  it("gzip-compresses pushed scrape results", async () => {
    ...
    expect(gunzipSync(init.body as Buffer).toString("utf8")).toBe(
  ...
  it("gzip-compresses pushed central logs and preserves Retry-After failures", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ ok: true, accepted: 1, duplicates: 0, invalid: 0 }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
    ...
    await expect(client.pushLogs(entries)).resolves.toEqual({
      ok: true,
      accepted: 1,
      duplicates: 0,
      invalid: 0,
  ...
  it("registers with the profile API key bootstrap credential", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ agentId: "agent-id", agentSecret: "agent-secret" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://dashboard.example/api/agent/register",
      {
        method: "POST",
        headers: {
  ...
  it("consumes a Mission Control pairing code without using the legacy register route", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ agentId: "paired-id", agentSecret: "paired-secret" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
    ...
    ).resolves.toEqual({ agentId: "paired-id", agentSecret: "paired-secret" });

    expect(fetchMock).toHaveBeenCalledWith(
      "https://dashboard.example/api/auth/agent-pairing/consume",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  it("shows human-readable pairing consume API errors", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
    ...
    ).rejects.toThrow("Pairing code not found.");
  });

  it("posts Mission Control progress with device credentials", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
    ...
    await client.pushProgress({
      agentJobPublicId: "abcdefghijkl",
      phase: "browser_opening",
      sourceUrl: "https://www.bolha.com/search?q=golf",
    });

    expect(fetchMock).toHaveBeenCalledWith(
      "https://dashboard.example/api/mission-control/progress",
      {
        method: "POST",
        headers: {
          "X-Agent-Id": "agent-id",
  ...
  it("revokes the current desktop app pairing with device credentials", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ ok: true }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
    ...
    await client.revokeCurrentDevice("paired_to_another_account");

    expect(fetchMock).toHaveBeenCalledWith(
      "https://dashboard.example/api/agent/devices/current/revoke",
      {
        method: "POST",
        headers: {
          "X-Agent-Id": "agent-id",
        ...
        body: JSON.stringify({ reason: "paired_to_another_account" }),
      },
    );
  });

  it("can abort desktop app revocation requests", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ ok: true }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
    ...
    await client.revokeCurrentDevice("agent_disconnected", {
  ...
  it("fetches a first-party WebSocket token with agent credentials", async () => {
    ...
    await expect(client.getWsToken()).resolves.toEqual({
      token: "ws-token",
      expiresAt: 123,
    });
    expect(fetchMock).toHaveBeenCalledWith(
      "https://dashboard.example/api/agent/ws-token",
      {
        headers: {
          "X-Agent-Id": "agent-id",
          "X-Agent-Secret": "agent-secret",
  ...
  it("throws a terminal credential error for revoked device credentials", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          error: "Invalid agent credentials",
          code: "agent_device_revoked",
          terminal: true,
          revocationReason: "dashboard_forget",
        }),
        {
          status: 401,
          headers: { "Content-Type": "application/json" },
    ...
    const error = await client.getWsToken().catch((err: unknown) => err);

    expect(error).toBeInstanceOf(TerminalAgentCredentialError);
    expect(isTerminalAgentCredentialError(error)).toBe(true);
