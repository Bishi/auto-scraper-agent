...
import {
  centralLogUploadDisabledForSession,
  flushCentralLogs,
...
export class Scheduler {
  ...
  private _fireImmediateHeartbeat:
    | ((wakeSource?: HeartbeatWakeSource) => void)
    | null = null;
  private _running = false;
  private _paused = false;
  private _stopRequested = false;
  private _started = false;
  private _terminalCredentialsInvalid = false;
  private _version = "";
  ...
  start(
    ...
    version = "",
    startPaused = false,
    options: { runStartupScrape?: boolean } = {},
  ): void {
    const runStartupScrape = options.runStartupScrape ?? true;
    this._started = true;
    this._terminalCredentialsInvalid = false;
    this._version = version;
    this._paused = startPaused;
    this.startHeartbeat(client);
    if (!startPaused && runStartupScrape) {
    ...
    this.wsClient = new AgentWebSocketClient(
      ...
      (commandId) => {
        this._fireImmediateHeartbeat?.(
          commandId === "connect" ? "ws_connect" : "ws_command",
        );
      },
      (reason) => {
        this.handleTerminalCredentialError(reason);
      },
    );
    this.wsClient.start();
  }
  ...
  private handleTerminalCredentialError(errOrReason: unknown): boolean {
    if (
      typeof errOrReason !== "string" &&
      !isTerminalAgentCredentialError(errOrReason)
    ) {
      return false;
    }
    const reason =
      typeof errOrReason === "string"
        ? errOrReason
        : (errOrReason.revocationReason ?? errOrReason.code);
    if (!this._terminalCredentialsInvalid) {
      this._terminalCredentialsInvalid = true;
      agentLogger.warn(
        `[config] Agent credentials are no longer valid (${reason}). Pair this desktop app again to resume scanning.`,
      );
      this.onTerminalCredentials(reason);
    }
    this.stop();
    return true;
  }
  ...
  stopScrape(): void {
    if (this._running) {
      ...
      agentLogger.info(
        "[scrape] Stop requested - will halt after current module completes",
      );
    } else {
      agentLogger.info("[scrape] Stop requested but no scrape is in progress");
    }
  }
  ...
  pause(): void {
    ...
    this._pausedRemainingMs =
      this.nextRunAt !== null ? Math.max(0, this.nextRunAt - Date.now()) : null;
    this.nextRunAt = null;
    this._paused = true;
  ...
  resume(client: AgentApiClient): void {
    ...
    const delay = Math.max(
      MIN_DELAY_MS,
      this._pausedRemainingMs ?? 30 * 60 * 1000,
    );
    this._pausedRemainingMs = null;

    this.nextRunAt = Date.now() + delay;
    agentLogger.info(
      `[scheduler] Resumed - next scrape in ${Math.round(delay / 60_000)} min`,
    );
    this.scrapeTimer = setTimeout(
      () => void this.runCycle(client, true, "schedule"),
      delay,
    );
  }
  ...
  async triggerNow(
    ...
    if (this._running) {
      agentLogger.info(
        "[scrape] Scrape already in progress - skipping manual trigger",
      );
      return;
    }
  ...
  private currentHeartbeatOptions(
    extra: {
      failureMsg?: string;
    ...
    return {
      ...
      ...(this._pendingAckCommandId
        ? { ackCommandId: this._pendingAckCommandId }
        : {}),
      ...(extra.failureMsg ? { failureMsg: extra.failureMsg } : {}),
      ...(extra.failureJobPublicId !== undefined
        ? { failureJobPublicId: extra.failureJobPublicId }
        : {}),
  ...
  private applyServerCommand(
    ...
    if (!command || !commandId || commandId === this._pendingAckCommandId)
      return;

    if (command === "flush_logs") {
      if (centralLogUploadDisabledForSession()) {
        agentLogger.warn(
          { component: "command", event: "logs_refresh_skipped" },
          "[command] Server command: flush_logs skipped because central log upload is disabled for this session",
        );
      } else {
        agentLogger.info(
          { component: "command", event: "logs_refresh_requested" },
          "[command] Logs refreshed on request",
        );
    ...
    if (command === "scrape_now") {
      if (!this._running) {
        if (commandPayload?.module) {
          agentLogger.info(
            `[command] Server command: scrape_now (${commandPayload.module})`,
          );
        } else {
          agentLogger.info("[command] Server command: scrape_now");
        }
    ...
    if (command === "check_update") {
      agentLogger.info("[command] Server command: check_update");
    ...
    if (command === "pause") {
      try {
        if (!this._paused) {
          this.pause();
          agentLogger.info("[command] Server command: pause");
        }
        this.stageAck(commandId);
      } catch (err) {
        agentLogger.error(
          "[command] Failed to apply pause command: " + String(err),
        );
      }
      return;
    }
    ...
    if (command === "resume") {
      try {
        if (this._paused) {
          agentLogger.info("[command] Server command: resume");
          this.resume(client);
        }
        this.stageAck(commandId);
      } catch (err) {
        agentLogger.error(
          "[command] Failed to apply resume command: " + String(err),
        );
      }
    }
  }
  ...
  private startHeartbeat(client: AgentApiClient): void {
    const beat = (wakeSource: HeartbeatWakeSource = "interval"): void => {
      ...
      if (wakeSource !== "interval") {
        agentLogger.info(
          { central: false, component: "heartbeat", wakeSource },
          `[heartbeat] Heartbeat wake source: ${wakeSource}`,
        );
      }
      client
        .heartbeat(
          this._version,
          process.platform,
          this.currentHeartbeatOptions({ wakeSource }),
        )
        .then((res) => {
          if (!this._started) return;
          if (this.transientHeartbeatFailures >= 3) {
            agentLogger.info("[heartbeat] Heartbeat recovered");
          }
          ...
          this.applyServerCommand(
            client,
            res.command,
            res.commandId,
            res.commandPayload ?? null,
          );
        ...
        .catch((err: unknown) => {
          if (this.handleTerminalCredentialError(err)) return;
          if (isTransientAgentApiError(err)) {
            this.transientHeartbeatFailures += 1;
            if (
              this.transientHeartbeatFailures === 3 ||
              (this.transientHeartbeatFailures > 3 &&
                this.transientHeartbeatFailures % 5 === 0)
            ) {
              agentLogger.warn(
                "[heartbeat] Heartbeat delayed: " + describeAgentApiError(err),
              );
            }
            return;
          }
          ...
          this.transientHeartbeatFailures = 0;
          agentLogger.error(
            "[heartbeat] Heartbeat failed: " + describeAgentApiError(err),
          );
        });
    };
  ...
  private async runCycle(
    ...
    try {
      ...
      const jobMap = new Map(
        schedule.jobs.map((j) => [j.moduleName, j.publicId]),
      );
      ...
      if (isTransientAgentApiError(err)) {
        ...
        agentLogger.warn("[scrape] Scrape cycle delayed: " + errMsg);
      } else {
        agentLogger.error("[scrape] Scrape cycle failed: " + errMsg);
        Promise.resolve(
          client.heartbeat(
            this._version,
            process.platform,
    ...
    if (scheduleNext && !this._paused && !this._terminalCredentialsInvalid) {
      this.nextRunAt = Date.now() + intervalMs;
      agentLogger.info(
        `[scheduler] Next scrape in ${Math.round(intervalMs / 60000)} min`,
      );
      this.scrapeTimer = setTimeout(
        () => void this.runCycle(client, true, "schedule"),
        intervalMs,
  ...
  private async scrapeAll(
    ...
    if (enabled.length === 0) {
      agentLogger.info("[scheduler] No modules enabled - nothing to scrape");
      return;
    }

    if (scope?.module != null && scopedEnabled.length === 0) {
      agentLogger.warn(
        `[scrape] Scoped scrape requested for unavailable module: ${scope.module}`,
      );
    ...
    if (scope?.module != null && !jobMap.has(scope.module)) {
      agentLogger.warn(
        `[scrape] Scoped scrape requested for ${scope.module}, but no queued job was returned`,
      );
      await client.cancelJobs([...jobMap.values()]);
      return;
    }
    ...
    agentLogger.info(
      `[scrape] ------------ Scrape started @ ${startedAt} [${triggerLabel}${scopeLabel}] ------------`,
    );
    ...
    for (const [moduleName, moduleConfig] of scopedEnabled) {
      if (this._stopRequested) {
        this._stopRequested = false;
        agentLogger.info(
          "[scrape] ------------ Scrape halted by user request ------------",
        );
        const cancelIds = [...jobMap.values()].filter(
          (id) => !startedJobPublicIds.has(id),
        );
        client.cancelJobs(cancelIds).catch((err: unknown) => {
          agentLogger.warn(
            "[scrape] Failed to cancel skipped jobs: " + String(err),
          );
        });
        break;
      }
      agentLogger.info(`[scrape] Scraping ${moduleName}...`);
      ...
      if (jobPublicId !== undefined) {
        ...
        try {
          ...
          await client
            .pushProgress({
              agentJobPublicId: jobPublicId,
              phase: "job_accepted",
              occurredAt: moduleStartedAt.toISOString(),
            })
            .catch(() => {});
        } catch (err: unknown) {
          if (this.handleTerminalCredentialError(err)) throw err;
          const errMsg = describeAgentApiError(err);
          agentLogger.error(
            `[scrape] Failed to mark job ${jobPublicId} as running: ${errMsg}`,
          );
          Promise.resolve(
            client.heartbeat(
              this._version,
      ...
      try {
        const streamScraperLog = (entry: LogEntry) =>
          pushScraperLog(moduleName, entry);
        const configuredUrls = Array.isArray(moduleConfig.urls)
          ? moduleConfig.urls
          : [];
        const firstUrl = configuredUrls[0];
        const primaryUrl =
        ...
        if (jobPublicId !== undefined) {
          ...
          await client
            .pushProgress({
              agentJobPublicId: jobPublicId,
              phase: "listings_parsed",
              sourceUrl: primaryUrl,
              context: { count: result.listings.length },
              occurredAt: new Date().toISOString(),
            })
            .catch(() => {});
        }
        ...
        if (result.hadManagedChallenge) {
          agentLogger.info(
            `[scrape] Retrying ${moduleName} with fresh browser profile...`,
          );
          ...
          const originalSnapshots = result.debugSnapshots.map((s) => ({
            ...s,
            preRetry: true,
          }));
          ...
          result = {
            ...result,
            debugSnapshots: [...originalSnapshots, ...result.debugSnapshots],
          };
        }

        if (jobPublicId === undefined) {
          throw new Error(
            `Missing scheduled job public id for module ${moduleName}`,
          );
        }
        ...
        try {
          ...
          if (isRateLimited) {
            agentLogger.warn(
              `[scrape] Rate limited by server - results not delivered for ${moduleName}. Will retry next scheduled run.`,
            );
          } else {
            agentLogger.error(
              `[scrape] Failed to push results for ${moduleName}: ${errMsg}`,
            );
          }
          Promise.resolve(
            client.heartbeat(
        ...
        agentLogger.info(
          `[scrape] ${moduleName}: total=${s.total} new=${s.new} changed=${s.changed} removed=${s.removed}`,
        );
      } catch (err) {
        if (this.handleTerminalCredentialError(err)) throw err;
        const errMsg = String(err);
        agentLogger.error(`[scrape] Failed to scrape ${moduleName}: ` + errMsg);
        Promise.resolve(
          client.heartbeat(
            this._version,
            process.platform,
    ...
    agentLogger.info(
      `[scrape] ------------ Scrape complete @ ${finishedAt} ------------`,
    );
  }
}
