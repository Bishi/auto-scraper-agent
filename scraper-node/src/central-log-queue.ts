...
function inferComponent(rawComponent: unknown, message: string): AgentLogComponent {
  if (isComponent(rawComponent)) return rawComponent;
  const prefix = message.match(/^\[([a-z_-]+)\]/i)?.[1]?.toLowerCase();
  if (prefix && prefix !== "agent" && isComponent(prefix)) return prefix;
  return "agent";
}
...
export function enqueueCentralAgentLog(raw: {
  ...
  const entry: CentralLogEntry = {
    ...
    component: inferComponent(component, msg),
