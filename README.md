...
A desktop tray app for Windows and macOS that runs scrape jobs on a schedule and reports results back to your Auto-Scraper server.
...
The full **End User License Agreement** text shown in the Windows NSIS installer is in [`src-tauri/EULA.txt`](src-tauri/EULA.txt).
...
The Rust shell handles the tray icon, setup window, and auto-update prompts. The Node.js sidecar does all the scraping and communicates with your Auto-Scraper server over HTTP using its registered `X-Agent-Id` and `X-Agent-Secret`. It also opens a first-party WebSocket with a short-lived token from `GET /api/agent/ws-token`; the socket carries command wake hints, while heartbeat remains the authoritative command fetch and ACK path. Heartbeats include a diagnostic wake source so the server can distinguish startup, WS connect, WS command wake, ACK follow-up, interval fallback, and failure reporting during soak tests. The two local processes talk to each other on `127.0.0.1:9001`, authenticated with an ephemeral shared secret generated on each sidecar startup — any other local process attempting to call the sidecar is rejected with `401 Unauthorized`.

Scraped data, phase telemetry, and heartbeats are sent to your server and surface across the dashboard, especially **Mission Control**, **Listings**, **Changes**, **Runs**, **Analytics**, and **Agent** tabs.
...
- Your running Auto-Scraper server with either a Mission Control pairing code or a legacy API key
...
On Windows, this runs two steps in sequence:
...
On macOS, run the same Chromium setup step on a Mac, then build the DMG:

```bash
npm run build:mac
```

The finished DMG lands under:
...
For the command, ACK, heartbeat, and first-party WebSocket contract specifically, see [docs/agent-command-lifecycle.md](docs/agent-command-lifecycle.md).
