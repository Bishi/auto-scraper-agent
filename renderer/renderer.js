...
async function mockInvoke(command, args = {}) {
  switch (command) {
    ...
    case "pair_config":
      ...
      mockState.hasAgentCredentials = true;
      mockState.apiKeyTail = "";
      mockLog("info", "Paired from Mission Control.");
      return null;
...
async function mockFetch(url, opts = {}) {
  ...
  if (pathname === "/config") {
    return { ok: true, status: 200, json: async () => ({ serverUrl: mockState.serverUrl, hasApiKey: mockState.hasApiKey, hasAgentCredentials: mockState.hasAgentCredentials, apiKeyTail: mockState.apiKeyTail }) };
  }
  if (pathname === "/pairing/consume" && method === "POST") {
    mockLog("info", "Pairing code consumed in mock mode.");
    mockState.hasApiKey = false;
    mockState.hasAgentCredentials = true;
...
function renderPairButton() {
  ...
  pairBtn.textContent = hasAgentCredentials
    ? pairSwitchConfirmArmed
      ? "Confirm switch account"
      : "Switch account"
    : "Pair Agent";
}

function resetPairSwitchConfirm() {
  pairSwitchConfirmArmed = false;
  renderPairButton();
}
...
serverInput.addEventListener("input", resetPairSwitchConfirm);
pairCodeInput.addEventListener("input", resetPairSwitchConfirm);
...
pairBtn.addEventListener("click", async () => {
  ...
  if (!/^\d{6}$/.test(code)) {
    errorEl.textContent = "Enter the 6-digit pairing code from Mission Control.";
    errorEl.style.display = "block";
    return;
  }
  if (hasAgentCredentials && !pairSwitchConfirmArmed) {
    pairSwitchConfirmArmed = true;
    renderPairButton();
    return;
  }
  ...
  try {
    const res = await fetchTimeout(`${SIDECAR}/pairing/consume`, 10_000, {
      method: "POST",
    ...
    pairCodeInput.value = "";
    successEl.textContent = "Paired. Agent will confirm with the server now.";
    successEl.style.display = "block";
    await loadConfig();
  } catch (err) {
