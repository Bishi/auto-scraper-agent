...
8. A module-scoped dashboard trigger runs exactly one module when the heartbeat command includes `commandPayload.module`, and the sidecar cancels the other queued jobs from that scrape cycle instead of scraping them
...
10. "Check for Updates" in tray shows "up to date" dialog when on latest version
