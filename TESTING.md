...
If no saved config: pair or POST one first (see step 3).
...
## 3. Pair or configure (first run or after wiping config)

Preferred Mission Control pairing:

```bash
curl -X POST http://127.0.0.1:9001/pairing/consume \
  -H "Content-Type: application/json" \
  -H "X-Sidecar-Token: $TOKEN" \
  -d '{"serverUrl":"http://localhost:3000","code":"123456"}'
```
...
Normal server calls after pairing or registration use `X-Agent-Id` and `X-Agent-Secret`; the profile API key is only the legacy setup/bootstrap credential.
...
the sidecar binary on the local machine first:

```bash
cd scraper-node
node build.mjs               # produces dist/scraper-node.exe on Windows, dist/scraper-node on macOS
./dist/scraper-node          # macOS/Linux; should print the listening line within 3 s
./dist/scraper-node.exe      # Windows; should print the listening line within 3 s
```
...
### Branch build (installer without tagging)
...
4. Run workflow. When it finishes (~10–20+ min), either:
   - Download artifact **`auto-scraper-agent-windows`** or **`auto-scraper-agent-macos-*`** from the run, or
   - Use the rolling **`latest`** pre-release (manual runs attach the NSIS installer and macOS DMG there; see URLs below).
...
- Same full pipeline as a tag build (tests, SEA sidecar, Tauri bundle); Windows signing runs if `WINDOWS_CERTIFICATE` is configured. macOS DMGs are currently ad-hoc signed for internal testing.
...
https://github.com/Bishi/auto-scraper-agent/releases/download/v{version}/Auto-Scraper.Agent_{version}_aarch64.dmg
https://github.com/Bishi/auto-scraper-agent/releases/download/v{version}/Auto-Scraper.Agent_{version}_x64.dmg
```
...
https://github.com/Bishi/auto-scraper-agent/releases/latest/download/Auto-Scraper.Agent_{version}_aarch64.dmg
https://github.com/Bishi/auto-scraper-agent/releases/latest/download/Auto-Scraper.Agent_{version}_x64.dmg
```
...
Returns JSON — `assets[].browser_download_url` contains the direct installer links.
...
https://github.com/Bishi/auto-scraper-agent/releases/download/latest/Auto-Scraper.Agent_{version}_aarch64.dmg
https://github.com/Bishi/auto-scraper-agent/releases/download/latest/Auto-Scraper.Agent_{version}_x64.dmg
```
...
14. Forget or revoke the paired device from the dashboard and confirm the agent logs one credentials-invalid message, stops heartbeat/WebSocket retry loops, clears saved device credentials, and returns to pairing-required state
15. "Check for Updates" in tray shows "up to date" dialog when on latest version
